...
for (const [view, h] of VIEWS) {
  for (const escuro of [false, true]) {
    ...
    // Uma tentativa às vezes sai em branco mesmo com o compositor forçado:
    // o PNG vem com o tamanho de um fundo liso. Em vez de confiar, MEDE — e
    // repete até a foto ter conteúdo. Foi assim que o board escuro passou
    // duas vezes despercebido.
    let ok = false;
    for (let tentativa = 1; tentativa <= 4 && !ok; tentativa++) {
      ...
      const bytes = Bun.file(png).size;
      ok = bytes > 100_000;
      if (!ok) console.log(`  · ${view} ${escuro ? 'escura' : 'clara'} saiu com ${bytes} bytes, repetindo`);
    }
    if (!ok) { console.error(`bloco ${view} não renderizou em 4 tentativas`); process.exit(1); }
